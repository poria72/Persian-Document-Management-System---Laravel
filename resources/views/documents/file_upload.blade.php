@extends('layouts.app')
@section('title',"بارگذاری ".ucfirst(config('settings.file_label_plural')))
@section('css')
    <style>
...
@section('content')
    <section class="content-header">
        <h1>
            بارگذاری {{ucfirst(config('settings.file_label_plural'))}} - <small>{{$document->name}}</small>
    ...
    <div class="content">
        ...
        <div style="text-align: center">
            <button type="button" class="btn btn-info btn-sm" onclick="addRow()"><i class="fa fa-plus"></i>
                اضافه کردن {{ucfirst(config('settings.file_label_singular'))}}
            </button>
            <button type="submit" class="btn btn-primary btn-sm"><i class="fa fa-save"></i> بارگذاری</button>
        </div>
        {!! Form::close() !!}
    </div>
@endsection
