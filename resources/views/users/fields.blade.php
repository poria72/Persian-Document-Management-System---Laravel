    ...
    <script id="permission-row" type="text/x-handlebars-template">
        <tr>
            ...
            @foreach (config('constants.TAG_LEVEL_PERMISSIONS')  as $perm)
                <td><label>
                        <input name="tag_permissions[@{{index}}][{{$perm}}]" type="checkbox"
                               value="1">
                    </label></td>
            @endforeach
            <td>
...
@can('user manage permission')
    <div class="box box-primary">
        <div class="box-header no-border">
            <h3 class="box-title">دسترسی های کلی</h3>
                        ...
                        <div class="col-sm-4">
                            ...
                            @foreach(config('constants.GLOBAL_PERMISSIONS.USERS') as $permission_name=>$permission_label)
                                <div class="form-group">
                                    <label>
                                    {{--  class="iCheck-helper" removed! --}}
                                        <input name="global_permissions[]" type="checkbox"
    ...
    <div class="box box-primary">
        <div class="box-header no-border">
            <h3 class="box-title">دسترسی مربوط به {{ucfirst(config('settings.tags_label_plural'))}}</h3>
            <div class="box-tools pull-right">
                <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
                </button>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <table class="table">
                        <thead>
                        <tr>
                            <th>انتخاب {{ucfirst(config('settings.tags_label_singular'))}}</th>
                            @foreach (config('constants.TAG_LEVEL_PERMISSIONS')  as $perm)
                                <th>{{ucfirst($perm)}}</th>
                            @endforeach
                        </tr>
                        ...
                        <tr>
                            <td colspan="6">
                                <button type="button" onclick="addRow()" class="btn btn-info btn-xs">اضافه کردن جدید {{config('settings.tags_label_singular')}}</button>
                            </td>
                        </tr>
                        </tfoot>
                    </table>
